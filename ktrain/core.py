...
def load_predictor(fpath, batch_size=U.DEFAULT_BS):
    ...
    try:
        ...
        try:
            ...
            warnings.warn('could load .preproc file as %s - attempting to load as %s' % (os.path.join(fpath, U.PREPROC_NAME), preproc_name))
            with open(preproc_name, 'rb') as f: preproc = pickle.load(f)
        except:
    ...
    try:
        try:
            ...
            try:
                ...
                warnings.warn('could not load %s - attempting to load model as %s' % (os.path.join(fpath, U.MODEL_NAME), fpath))
