...
maxlen = p.preproc.maxlen
tokenizer = p.preproc.get_tokenizer()
input_dict = tokenizer(doc, max_length=maxlen, 
                       return_tensors='tf', padding='max_length')
feed = {}
feed['input_ids'] = np.array(input_dict['input_ids']).astype('int32')
feed['attention_mask'] = np.array(input_dict['attention_mask']).astype('int32')
output_onnx = sess.run(None, feed)
print("predicted class: %s" % ( p.get_classes()[np.argmax(output_onnx[0][0])]) )
# output:
